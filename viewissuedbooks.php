...
<?php } ?>
	...
	{ 
		...
		echo "<tr>
			...
			<td>$rs[bookname]<br>
			<img src='imgbook/$rs[bookimg]' style='width: 150px;height:150px;'></td>
			<td>" . date("d-M-Y",strtotime($rs['borrowdate'])) . "<br>" . date("h:i A",strtotime($rs['borrowdate'])) ."</td>
			<td>" . date("d-M-Y h:i A",strtotime($rs['returndate'])) ."<hr>";
			if(floor($difference/60/60/24) >= 0)
			{
				echo "<b>". floor($difference/60/60/24)." days remaining</b>";
			}
			else
			{
				echo "<b style='color: red;'>". floor($difference/60/60/24)." days remaining</b>";
			}			
			echo "</td>";
		if(isset($_SESSION['librarian_id']))
		{
			echo "<td><a  class='btn btn-info' href='penalty.php?transactionid=$rs[0]' >Return<br>Book</a></td>";
		}
		echo "</tr>";
	}
	?>
