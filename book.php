...
if(isset($_POST['submit']))
{
	...
	{
		//4. update statement starts 
		$sql = "UPDATE book SET bookcategoryid ='$_POST[bookcategoryid]',bookname='" . mysqli_real_escape_string($con,$_POST['bookname']) ."',callnumber='$_POST[callnumber]',bookdescription='" . mysqli_real_escape_string($con,$_POST['bookdescription']) ."',bookimg='$filename',barcode='$_POST[barcode]',bookcost='$_POST[bookcost]',author='" . mysqli_real_escape_string($con,$_POST['author']) ."',status='$_POST[status]' WHERE bookid='$_GET[editid]'";
	...
	else
	{
		...
		$sql = "INSERT INTO book(bookcategoryid,bookname,callnumber,bookdescription,bookimg,bookcost,author,barcode,status) VALUES  ('$_POST[bookcategoryid]','" . mysqli_real_escape_string($con,$_POST['bookname']) ."','$_POST[callnumber]','" . mysqli_real_escape_string($con,$_POST['bookdescription']) ."','$filename','$_POST[bookcost]','$_POST[barcode]','" . mysqli_real_escape_string($con,$_POST['author']) ."','$_POST[status]')";
		$qsql = mysqli_query($con,$sql);
		echo mysqli_error($con);
		if(mysqli_affected_rows($con) ==1);
		{
...
<form id="contact-form" role="form" action="" method="post" enctype="multipart/form-data" onsubmit="return validatedata()" name="frmform">
	<div class="col-md-6 col-sm-12">
			  ...
			  <div class="col-md-12 col-sm-12">
				...
				<input type="text" class="form-control" placeholder="Enter book title" name="bookname" value="<?php echo $rsedit['bookname']; ?>" > 

<label>Call number: <span class="err" id="idcallnumber"></span></label>
				<input type="text" class="form-control" placeholder="Enter callnumber" name="callnumber" value="<?php echo $rsedit['callnumber']; ?>" > 
				...
				<input type="text" class="form-control" placeholder="Enter Book author" name="author" value="<?php echo $rsedit['author']; ?>" >  
                 ...
                 <label>Barcode: <span class="err" id="idbarcode"></span></label>
				<input type="text" class="form-control" placeholder="Enter barcode" name="barcode" value="<?php echo $rsedit['barcode']; ?>" > 
...
function validatedata()
{
	...
	$( ".err" ).empty();
	 ...
	 {
		 document.getElementById("idbookname").innerHTML= "Please enter book title...";
		 condition = "False";
	 }
	 if(document.frmform.callnumber.value == "")
	 {
		 document.getElementById("idcallnumber").innerHTML= "Please enter call number...";
		 condition = "False";
	 }
	 ...
	 if(document.frmform.barcode.value == "")
	 {
		 document.getElementById("idbarcode").innerHTML= "Please enter barcode...";
		 condition = "False";
	 }
